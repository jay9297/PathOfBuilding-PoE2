...
function SkillSetListClass:CreateSkillSet()
	...
	controls.save = new("ButtonControl", nil, { -45, 70, 80, 20 }, "Save", function()
		-- Snapshot existing sets before creation so the new set can be seeded with
		-- the next level bracket for the PoE2 .build export (see BuildExportPoE2).
		local existing = {}
		for _, id in ipairs(self.skillsTab.skillSetOrderList) do
			t_insert(existing, self.skillsTab.skillSets[id])
		end
		local newSet = self.skillsSetService:NewSkillSet(controls.edit.buf)
		local ok, BuildExportPoE2 = pcall(require, "Modules/BuildExportPoE2")
		if ok and newSet then
